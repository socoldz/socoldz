...
{
public:
    Rectang(int start, int width, int height) :
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_CREATE: {
        figures.push_back(new Circle(125, 125));
        figures.push_back(new Rectang(300, 400, 250));
        break;  
    }
    case WM_COMMAND:
        {
    ...
    case WM_PAINT:
        {
            ...
            for (std::vector <Shape*>::iterator i = figures.begin(); i != figures.end(); ++i)
            {
                (*i)->Draw(hdc);
            }
            EndPaint(hWnd, &ps);
